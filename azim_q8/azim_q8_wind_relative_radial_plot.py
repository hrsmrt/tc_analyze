...
sample_data = np.load(
    f"{config.get_data_path('azim_q8', 'wind_relative_radial')}/t{str(config.t_first).zfill(3)}.npy"
)
...
def process_t(t):
    # データの読み込み
    data = np.load(f"{config.get_data_path('azim_q8', 'wind_relative_radial')}/t{str(t).zfill(3)}.npy")
