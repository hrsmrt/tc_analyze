...
def refactor_calc_file(filepath):
    ...
    if content == original_content:
        content = re.sub(OLD_SETTING_BLOCK_ALT, NEW_CONFIG_IMPORTS, content, flags=re.MULTILINE)
    ...
    if 'json.load' not in content and 'json.dump' not in content and 'import json' in content:
...
config = AnalysisConfig()'''
        ...
        if content == original_content:
            content = re.sub(OLD_SETTING_BLOCK_ALT, config_imports, content, flags=re.MULTILINE)
