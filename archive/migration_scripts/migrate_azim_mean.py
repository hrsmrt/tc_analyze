...
def migrate_file(filepath):
    ...
    needs_grid = (
        "X, Y = np.meshgrid" in content
        or "grid.X" in content
        or "vgrid = np.loadtxt" in content
    )
    ...
    if (
        is_plot_file
        and "from utils.plotting import parse_style_argument" not in content
    ):
    ...
    if is_plot_file:
        ...
        if (
            "set_xticks([0,250e3,500e3,750e3,1000e3]" in content
            or "set_xticks([0e3,250e3,500e3,750e3,1000e3]" in content
        ):
