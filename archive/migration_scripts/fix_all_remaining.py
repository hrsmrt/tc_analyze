...
def fix_file(filepath: Path) -> bool:
    ...
    if "with open('setting.json'" not in content and "with open(\"setting.json\"" not in content:
        return False
    ...
    if 'sys.path.append' not in content:
        ...
        if import_section_end > 0:
            ...
            lines = lines[:import_section_end+1] + sys_path_lines + lines[import_section_end+1:]
            content = '\n'.join(lines)
...
config = AnalysisConfig()'''
    ...
    if not replaced and ("with open('setting.json'" in content or 'with open("setting.json"' in content):
        lines = content.split('\n')
        new_lines = []
        skip = False
        imports_added = False
        ...
        for i, line in enumerate(lines):
            ...
            if not in_setting_block and (
                '# ファイルを開いてJSONを読み込む' in line or
                (not skip and ("with open('setting.json'" in line or 'with open("setting.json"' in line))
            ):
            ...
            if skip:
                ...
                # End patterns: empty line followed by non-setting code, or specific variable assignments
                stripped = line.strip()
                if (
                    (stripped == '' and i+1 < len(lines) and lines[i+1].strip() and
                     not any(var in lines[i+1] for var in ['glevel', 'nt', 'dt', 'triangle_size', 'nx', 'ny', 'nz', 'x_width', 'y_width', 'dx', 'dy', 'input_folder', 'setting'])) or
                    stripped.startswith('folder =') or
                    stripped.startswith('output_folder =') or
                    stripped.startswith('time_list =') or
                    stripped.startswith('r_max =') or
                    stripped.startswith('z_max =') or
                    stripped.startswith('def ') or
                    stripped.startswith('nr =') or
                    stripped.startswith('R =') or
                    stripped.startswith('f =') or
                    'center_x_list' in stripped or
                    'center_y_list' in stripped or
                    stripped.startswith('vgrid =') or
                    stripped.startswith('xgrid =') or
                    'X, Y =' in stripped or
                    'X,Y =' in stripped or
                    stripped.startswith('x =') or
                    stripped.startswith('y =') or
                    (stripped.startswith('pres_s =') and 'setting' not in stripped) or
                    (stripped.startswith('Rd =') and 'setting' not in stripped) or
                    (stripped.startswith('Cp =') and 'setting' not in stripped) or
                    (stripped.startswith('L =') and 'setting' not in stripped)
                ):
                    skip = False
                    in_setting_block = False
    ...
    if 'json.load' not in content and 'json.dump' not in content and 'json.loads' not in content:
    ...
    if is_plot_file and 'sys.argv' in content and 'mpl_style_sheet' in content:
        ...
        if re.search(old_style_pattern, content):
            content = re.sub(old_style_pattern, 'mpl_style_sheet = parse_style_argument()\n', content)
        else:
            # Try simpler pattern
            old_style_pattern2 = r'if len\(sys\.argv\) > [12]:[^\n]*\n[^\n]*mpl_style_sheet[^\n]*\n[^\n]*print[^\n]*\nelse:[^\n]*\n[^\n]*print[^\n]*\n'
            content = re.sub(old_style_pattern2, 'mpl_style_sheet = parse_style_argument()\n', content)
