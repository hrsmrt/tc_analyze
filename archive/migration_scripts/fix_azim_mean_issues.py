...
def fix_file(filepath):
    ...
    if re.search(old_style_pattern, content):
        ...
        # Replace the pattern
        content = re.sub(old_style_pattern, 'mpl_style_sheet = parse_style_argument(arg_index=1)', content)
    ...
    if 'X*1e-3, Y*1e-3' in content or 'X, Y' in content:
        if 'X, Y = grid.create_radial_vertical_meshgrid' not in content and \
           'X, Y = grid.X, grid.Y' not in content and \
           'X, Y = np.meshgrid' not in content:
