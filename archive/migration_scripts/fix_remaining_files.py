...
config = AnalysisConfig()'''
    ...
    if not replaced and "with open('setting.json'" in content:
        # Find and remove lines from "# ファイルを開いて" or "with open" to "config.dx = config.x_width / config.nx"
        lines = content.split('\n')
        new_lines = []
        skip = False
        imports_added = False
        ...
        for i, line in enumerate(lines):
            ...
            if skip:
                ...
                if (line.strip() == '' or
                    line.startswith('time_list') or
                    line.startswith('r_max') or
                    line.startswith('output_folder') or
                    line.startswith('folder') or
                    line.startswith('def ') or
                    line.startswith('nr =') or
                    line.startswith('R =') or
                    line.startswith('f =') or
                    line.startswith('pres_s =') or
                    line.startswith('Rd =') or
                    line.startswith('Cp =') or
                    line.startswith('L =') or
                    line.startswith('vgrid =') or
                    line.startswith('xgrid =') or
                    line.startswith('X, Y =')):
                    skip = False
