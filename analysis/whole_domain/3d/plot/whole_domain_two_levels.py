"""Plot 3D data over whole domain with two different z-levels overlaid (both as line contours)."""
# python $WORK/tc_analyze/analysis/whole_domain/3d/plot/whole_domain_two_levels.py varname z1 z2 $style [sigma]
# 例: python $WORK/tc_analyze/3d/whole_domain_two_levels.py ms_tem 0 36 $style
# 例（smooth化）: python $WORK/tc_analyze/3d/whole_domain_two_levels.py ms_tem 0 36 $style 2.0
import os
import sys
...
VARNAME = sys.argv[1]
Z1 = int(sys.argv[2])  # 1つ目のZ面（赤い線のcontour）
Z2 = int(sys.argv[3])  # 2つ目のZ面（青い線のcontour）
...
if SIGMA > 0:
    OUTPUT_DIR = config.get_domain_path("whole_domain", os.path.join(f"3d/whole_domain_two_levels/{VARNAME}", f"_z{Z1}_z{Z2}_smooth{SIGMA:.1f}"), data_type="fig")
else:
    OUTPUT_DIR = config.get_domain_path("whole_domain", os.path.join(f"3d/whole_domain_two_levels/{VARNAME}", f"_z{Z1}_z{Z2}"), data_type="fig")
...
def process_t(t):
    ...
    if settings1['levels'] is not None:
        # 線の数を減らして見やすくする
        levels1 = settings1['levels'][::2] if len(settings1['levels']) > 10 else settings1['levels']
        cs1 = ax.contour(
            X, Y, data1,
            levels=levels1,
            colors='red',
        ...
        cs1 = ax.contour(X, Y, data1, colors='red', linewidths=1.5, alpha=0.9)
    ...
    if settings2['levels'] is not None:
        ...
        cs2 = ax.contour(X, Y, data2, colors='blue', linewidths=1.5, alpha=0.9)
    ...
    legend_elements = [
        Line2D([0], [0], color='red', linewidth=1.5,
               label=f'z={vgrid[Z1] * 1e-3:.1f}km'),
        Line2D([0], [0], color='blue', linewidth=1.5,
               label=f'z={vgrid[Z2] * 1e-3:.1f}km')
    ]
