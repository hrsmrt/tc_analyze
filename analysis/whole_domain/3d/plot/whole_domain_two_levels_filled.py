...
if SIGMA > 0:
    OUTPUT_DIR = config.get_domain_path("whole_domain", os.path.join(f"3d/whole_domain_two_levels_filled/{VARNAME}", f"_z{Z1}_z{Z2}_smooth{SIGMA:.1f}"), data_type="fig")
else:
    OUTPUT_DIR = config.get_domain_path("whole_domain", os.path.join(f"3d/whole_domain_two_levels_filled/{VARNAME}", f"_z{Z1}_z{Z2}"), data_type="fig")
