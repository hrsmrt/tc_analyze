...
output_folder = config.get_tc_centric_path("diagnostics", "symmetrisity/relative_wind_tangential")
...
def process_t(t):
    ...
    data_azim_mean = np.load(
        os.path.join(config.get_tc_centric_path('azimuthal', 'basic/wind_relative_tangential'), f"t{str(t).zfill(3)}.npy")
    )
    ...
    data = np.load(os.path.join(config.get_tc_centric_path('vortex_region', '3d/ms_wind_relative_tangential'), f"t{str(t).zfill(3)}.npy"))
