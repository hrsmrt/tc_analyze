...
OUTPUT_DIR = config.get_fig_path("3d", "vortex_region", "wind_tangential")
...
for z in z_list:
    os.makedirs(
        os.path.join(OUTPUT_DIR, f"z{str(z).zfill(2)}"), exist_ok=True
    )
...
def process_t(t):
    data_t = np.load(os.path.join(config.get_data_path("3d", "wind_tangential"), f"t{str(t).zfill(3)}.npy"))
