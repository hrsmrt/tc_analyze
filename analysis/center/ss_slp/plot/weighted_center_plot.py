"""Plot tropical cyclone center trajectory (weighted_centroid method).

Plots the trajectory of TC center calculated with weighted_centroid method.
Reads from weighted_center.npz (or configured filename in center_configs).

Usage:
    python $WORK/tc_analyze/analysis/center/ss_slp/plot/weighted_center_plot.py
    python $WORK/tc_analyze/analysis/center/ss_slp/plot/weighted_center_plot.py dark_background
"""
import os
import matplotlib
...
if not os.path.exists(filepath):
    raise FileNotFoundError(
        ...
        "Please run: python analysis/center/ss_slp/calc/weighted.py"
    )
...
title = "TC Center Trajectory (weighted_centroid)"
if r_refine is not None:
    title += f"\nr_refine={r_refine*1e-3:.0f}km"
...
output_path = os.path.join(output_dir, "trajectory_weighted.png")
