"""Plot horizontal distribution of tropical cyclone center by height (weighted_centroid method, z≤20km).

Plots the TC center positions on x-y plane at each time step, colored by height.
Shows how center location varies with height in horizontal plane (z ≤ 20 km only).
Reads from weighted_center.npz (or configured filename in center_configs).

Usage:
    python $WORK/tc_analyze/analysis/center/ms_pres/plot/weighted_center_horizontal_plot_20km.py
    python $WORK/tc_analyze/analysis/center/ms_pres/plot/weighted_center_horizontal_plot_20km.py dark_background
"""
import os
import matplotlib
...
# Filter to z ≤ 20 km
z_max_km = 20.0
z_mask = vgrid_km <= z_max_km
nz_plot = np.sum(z_mask)
vgrid_km_plot = vgrid_km[z_mask]
print(f"Plotting z ≤ {z_max_km} km: {nz_plot} levels (heights: {vgrid[z_mask][0]:.0f} to {vgrid[z_mask][-1]:.0f} m)")
...
def process_t(t):
    """Plot horizontal distribution for a single time step (z ≤ 20 km)."""
    # Extract only z ≤ 20 km
    x_centers = center[t, z_mask, 0] * 1e-3  # Convert to km
    y_centers = center[t, z_mask, 1] * 1e-3  # Convert to km
    ...
    title = f"TC Center by Height (weighted_centroid, z≤{z_max_km}km)\nt = {time_hour:.0f} hour"
    if r_search is not None and r_refine is not None:
        title += f"\nr_search={r_search*1e-3:.0f}km, r_refine={r_refine*1e-3:.0f}km"
    ax.set_title(title, fontsize=10)

    output_path = os.path.join(output_dir, f"horizontal_weighted_20km_t{str(t).zfill(3)}.png")
...
print(f"Saved horizontal scatter plots (z≤{z_max_km}km): {output_dir}/horizontal_weighted_20km_t*.png")
...
print(f"Z-range: {z_first} to {z_last}, plotted: 0 to {vgrid[z_mask][-1]:.0f} m ({z_max_km} km)")
