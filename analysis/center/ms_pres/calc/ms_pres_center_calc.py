...
1. weighted_centroid: Search within r_search, then refine with weighted centroid
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=f"Initial search radius from ss_slp center for weighted_centroid method (default: {R_SEARCH_DEFAULT:.0f})",
    )
    parser.add_argument(
        "--r-refine",
        "-rr",
    ...
    for z_idx, z in enumerate(range(z_first, z_last + 1)):
        ...
        results = Parallel(n_jobs=config.n_jobs)(
            delayed(process_t)(
                ...
                method, r_search, r_refine, r_smooth, refine_after_smooth
            )
...
def process_t(
    ...
    Args:
        ...
        r_search: Initial search radius from ss_slp center (weighted_centroid only)
        r_refine: Refinement radius for weighted centroid method
        r_smooth: Smoothing radius (smoothed_minimum only)
