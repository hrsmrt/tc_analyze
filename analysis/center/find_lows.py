...
# python $WORK/tc_analyze/analysis/center/find_lows.py
import os
import pickle
...
def main():
    ...
    for t in range(config.t_first, config.t_last + 1):
        ...
        lows = find_lows_peakmax(
            P,
            thr_Pa=100500,
            dx_km=config.dx * 1e-3,
            dy_km=config.dy * 1e-3,
            min_sep_km=200,
        )
...
def find_lows_peakmax(
    P, land_sea_mask=None, sigma=1.0, min_sep_km=200, dx_km=25, dy_km=25, thr_Pa=None
):
    # 1) 平滑化（sigmaはグリッド単位）
    ...
    lows = []
    for i, j in coords:
        lows.append({"iy": int(i), "ix": int(j), "p": float(Ps[i, j])})  # 平滑後の値
    # 圧力の小さい順に並べる
