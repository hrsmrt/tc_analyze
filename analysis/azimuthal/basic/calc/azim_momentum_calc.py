...
# input: os.path.join(config.get_data_path('azim', 'wind_tangential'), f"t{str(t).zfill(3)}.npy")
...
def process_t(t):
    u_tangential = np.load(os.path.join(config.get_data_path('azim', 'wind_tangential'), f"t{str(t).zfill(3)}.npy"))
    # データの形状から半径方向のビン数を取得
