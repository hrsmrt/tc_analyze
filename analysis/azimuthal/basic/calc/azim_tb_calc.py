# python $WORK/tc_analyze/analysis/azimuthal/basic/calc/azim_tb_calc.py
import os
...
folder1 = config.get_tc_centric_path("azimuthal", "basic/tb_radial")
folder2 = config.get_tc_centric_path("azimuthal", "basic/tb_tangential")
...
data_all_u = np.memmap(
    f"{config.input_folder}ms_tb_du.grd",
    dtype=">f4",
    mode="r",
    shape=(config.nt, config.nz, config.ny, config.nx),
)
data_all_v = np.memmap(
    f"{config.input_folder}ms_tb_dv.grd",
    dtype=">f4",
    mode="r",
    shape=(config.nt, config.nz, config.ny, config.nx),
)
...
def process_t(t):
    ...
    print(
        f"azim mean data t: {t}, max: {azim_mean_tangential.max()}, "
        f"min: {azim_mean_tangential.min()}"
    )
    np.savez(
        os.path.join(folder2, f"t{str(t).zfill(3)}.npz"),
        data=azim_mean_tangential,
        varname="tb_tangential",
        r_max=r_max,
        dx=config.dx,
        method="azimuthal_mean",
        created_at=datetime.now().isoformat()
