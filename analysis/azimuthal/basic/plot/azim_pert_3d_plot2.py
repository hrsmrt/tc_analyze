# python $WORK/tc_analyze/analysis/azimuthal/basic/plot/azim_pert_3d_plot2.py varname $style
# 方位角平均データをプロット
import os
import sys
...
folder = config.get_tc_centric_path("azimuthal", f"basic/pert2/{varname}", data_type="fig")
...
def process_t(t):
    # データの読み込み
    data = np.load(os.path.join(config.get_tc_centric_path('azimuthal', f'basic/pert2/{varname}'), f"t{str(t).zfill(3)}.npy"))
    ...
    match varname:
        case "ms_tem":
            ...
            ax.grid(
                which="minor", axis="both", color="gray", linestyle=":", linewidth=0.5
            )
            title_name = "気温偏差"
        case "ms_pres":
        ...
        case "ms_rho":
            c = ax.contourf(
                X, Y, data * 1e2, levels=20, cmap="seismic", vmax=1.8, vmin=-1.8
            )
            cbar = fig.colorbar(c, ax=ax)
            ax.set_xticks([0, 1000])
            ax.minorticks_on()
