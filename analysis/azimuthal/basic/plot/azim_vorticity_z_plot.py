...
output_folder = config.get_tc_centric_path("azimuthal", "basic/vorticity_z", data_type="fig")
os.makedirs(output_folder, exist_ok=True)


def process_t(t):
    # データの読み込み（.npz優先、.npyフォールバック）
    data_path = config.get_tc_centric_path("azimuthal", "basic/vorticity_z")
    ...
    c = ax.contourf(
        X, Y, data, levels=np.arange(-0.001, 0.0011, 0.0001), cmap="bwr", extend="both"
    )
