...
sample_data = np.load(os.path.join(config.get_tc_centric_path("azimuthal", "basic/dyn_radial"), f"t{str(config.t_first).zfill(3)}.npy"))
...
def process_t(t):
    data = np.load(os.path.join(config.get_tc_centric_path("azimuthal", "basic/dyn_radial"), f"t{str(t).zfill(3)}.npy"))
