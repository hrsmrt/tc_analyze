...
sample_data = np.load(os.path.join(config.get_data_path('azim', 'wind_relative_radial'), f"t{str(config.t_first).zfill(3)}.npy"))
...
def process_t(t):
    radial_wind_data = np.load(
        os.path.join(config.get_data_path('azim', 'wind_relative_radial'), f"t{str(t).zfill(3)}.npy")
    )
