...
sample_data = np.load(os.path.join(config.get_data_path('azim', 'mp_tangential'), f"t{str(config.t_first).zfill(3)}.npy"))
nr = sample_data.shape[1]
R_MAX = nr * config.dx
r_mesh, z_mesh = grid.create_radial_vertical_meshgrid(R_MAX)

folder = config.get_fig_path("azim", "mp_tangential")

os.makedirs(folder, exist_ok=True)


def process_t(t):
    data = np.load(os.path.join(config.get_data_path('azim', 'mp_tangential'), f"t{str(t).zfill(3)}.npy"))
