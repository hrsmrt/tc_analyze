# python $WORK/tc_analyze/analysis/azimuthal/q8/plot/azim_q8_wind_relative_radial_plot.py $style
import os
...
sample_data = np.load(
    os.path.join(config.get_data_path('azim_q8', 'wind_relative_radial'), f"t{str(config.t_first).zfill(3)}.npy")
)
...
folder = config.get_fig_path("azim_q8", "wind_relative_radial")
...
def process_t(t):
    # データの読み込み
    data = np.load(os.path.join(config.get_data_path('azim_q8', 'wind_relative_radial'), f"t{str(t).zfill(3)}.npy"))
