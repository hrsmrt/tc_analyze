...
sample_data = np.load(os.path.join(config.get_tc_centric_path("azimuthal", "basic/eliassen"), f"B/t{str(config.t_first).zfill(3)}.npy"))
...
def process_t(t):
    data = np.load(os.path.join(config.get_tc_centric_path("azimuthal", "basic/eliassen"), f"B/t{str(t).zfill(3)}.npy"))
