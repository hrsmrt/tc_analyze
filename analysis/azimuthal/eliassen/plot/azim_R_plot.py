...
sample_data = np.load(os.path.join(config.get_data_path('azim', 'eliassen'), f"R/t{str(config.t_first).zfill(3)}.npy"))
nr = sample_data.shape[1]
R_MAX = nr * config.dx
r_mesh, z_mesh = grid.create_radial_vertical_meshgrid(R_MAX)

output_folder = config.get_fig_path("azim", "eliassen", "R")
os.makedirs(output_folder, exist_ok=True)


def process_t(t):
    data = np.load(os.path.join(config.get_data_path('azim', 'eliassen'), f"R/t{str(t).zfill(3)}.npy"))
