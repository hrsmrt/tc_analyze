...
sample_data = np.load(os.path.join(config.get_tc_centric_path("azimuthal", "basic/eliassen"), f"N2/t{str(config.t_first).zfill(3)}.npy"))
nz_data, nr = sample_data.shape
R_MAX = nr * config.dx
r_mesh, z_mesh = grid.create_radial_vertical_meshgrid(R_MAX, nz=nz_data)

output_folder = config.get_tc_centric_path("azimuthal", "eliassen/N2", data_type="fig")
os.makedirs(output_folder, exist_ok=True)


def process_t(t):
    data = np.load(os.path.join(config.get_tc_centric_path("azimuthal", "basic/eliassen"), f"N2/t{str(t).zfill(3)}.npy"))
    ...
    ax.set_ylim([0, 20])
    ax.set_title(f"$N^2$ t = {config.time_list[t]} hour")
    ax.set_xlabel("半径 [km]")
    ax.set_ylabel("高度 [km]")
