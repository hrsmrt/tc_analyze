# python $WORK/tc_analyze/analysis/azimuthal/momentum/u/plot/azim_gradient_wind_eq_plot.py $style
import os
...
sample_data = np.load(os.path.join(config.get_tc_centric_path("azimuthal", "momentum/u"), f"gradient_wind_eq/t{str(config.t_first).zfill(3)}.npy"))
...
output_folder = config.get_tc_centric_path("azimuthal", "momentum/u/gradient_wind_eq", data_type="fig")

os.makedirs(output_folder, exist_ok=True)


def process_t(t):
    data = np.load(os.path.join(config.get_tc_centric_path("azimuthal", "momentum/u"), f"gradient_wind_eq/t{str(t).zfill(3)}.npy"))
