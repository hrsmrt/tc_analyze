...
sample_data = np.load(os.path.join(config.get_data_path('azim', 'eq_momentum_u'), f"gradient_wind_eq/t{str(config.t_first).zfill(3)}.npy"))
...
output_folder = config.get_fig_path("azim", "eq_momentum_u", "gradient_wind_eq")

os.makedirs(output_folder, exist_ok=True)


def process_t(t):
    data = np.load(os.path.join(config.get_data_path('azim', 'eq_momentum_u'), f"gradient_wind_eq/t{str(t).zfill(3)}.npy"))
