...
sample_data = np.load(os.path.join(config.get_tc_centric_path("azimuthal", "momentum/u/grad_p"), f"t{str(config.t_first).zfill(3)}.npy"))
...
output_folder = config.get_tc_centric_path("azimuthal", "momentum/u/grad_p", data_type="fig")

os.makedirs(output_folder, exist_ok=True)


def process_t(t):
    data = np.load(os.path.join(config.get_tc_centric_path("azimuthal", "momentum/u/grad_p"), f"t{str(t).zfill(3)}.npy"))

    plt.style.use(mpl_style_sheet)
    fig, ax = plt.subplots(figsize=(5, 2))
    c = ax.contourf(
        X, Y, data, cmap="bwr", levels=np.arange(-0.05, 0.055, 0.005), extend="both"
    )
    cbar = fig.colorbar(c, ax=ax)
    cbar.set_ticks([-0.05, 0, 0.05])
    ax.set_title(f"気圧傾度力 t = {config.time_list[t]} hour")
    ax.set_ylim([0, 20])
    ax.set_xlabel("半径 [km]")
    ax.set_ylabel("高度 [km]")
