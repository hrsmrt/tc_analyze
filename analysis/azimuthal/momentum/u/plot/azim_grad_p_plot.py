...
sample_data = np.load(os.path.join(config.get_data_path('azim', 'eq_momentum_u', 'grad_p'), f"t{str(config.t_first).zfill(3)}.npy"))
...
def process_t(t):
    data = np.load(os.path.join(config.get_data_path('azim', 'eq_momentum_u', 'grad_p'), f"t{str(t).zfill(3)}.npy"))
