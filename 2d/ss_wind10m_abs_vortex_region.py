...
ss_u10m = np.fromfile("../model/convert/ss_u10m.grd",dtype=">f4").reshape(nt,ny,nx)
ss_v10m = np.fromfile("../model/convert/ss_v10m.grd",dtype=">f4").reshape(nt,ny,nx)
...
def process_t(t):
    ...
    data_cut = data_t[np.ix_(y_idx, x_idx)]
    ...
    ax.set_title(f"10m風速 [m/s] t={t*dt_hour}h")
