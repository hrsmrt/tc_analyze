...
sample_data = np.load(f"{config.get_data_path('azim', 'wind_relative_radial')}/t{str(config.t_first).zfill(3)}.npy")
...
def process_t(t):
    radial_wind_data = np.load(
        f"{config.get_data_path('azim', 'wind_relative_radial')}/t{str(t).zfill(3)}.npy"
    )
