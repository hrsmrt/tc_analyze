...
vgrid = vgrid*1e-3
rgrid_wall = np.array([ r * dx + dx/2 for r in range(1,int(nr)-1)]) * 1e-3
...
def process_t(t):
  data = np.load(f"./data/azim/eq_momentum_u/gradient_balance_score/t{str(t).zfill(3)}.npy")

  plt.style.use(mpl_style_sheet)
  fig, ax = plt.subplots(figsize=(5,2))
  c = ax.contourf(X,Y,data,cmap="rainbow",levels=np.arange(0,1.1,0.1),extend="both")
  cbar = fig.colorbar(c, ax=ax)
  cbar.set_ticks([0,1])
  ax.set_title(f"傾度風平衡 t = {time_list[t]} hour")
  ax.set_ylim([0, 20])
  ax.set_xlabel("半径 [km]")
  ax.set_ylabel("高度 [km]")
