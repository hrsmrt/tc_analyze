...
sample_data = np.load(f"{config.get_data_path('azim', 'eq_momentum_u')}/du_dr/t{str(config.t_first).zfill(3)}.npy")
...
def process_t(t):
    data = np.load(f"{config.get_data_path('azim', 'eq_momentum_u')}/du_dr/t{str(t).zfill(3)}.npy")
