...
input_folder = "./data/azim/theta_e/"
...
def process_t(t):
    ...
    c = ax.contourf(X*1e-3, Y*1e-3, data, levels=np.arange(330,375,5), cmap="rainbow", extend="both")
    cbar = fig.colorbar(c,ax=ax)
    cbar.set_ticks([330,370])
    ax.set_ylim([0,20])
    ax.set_title(f"相当温位 t = {time_list[t]} hour")
    ax.set_xlabel("半径 [km]")
    ax.set_ylabel("高度 [km]")
