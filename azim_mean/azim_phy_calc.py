...
folder1 = f"./data/azim/phy_radial/"
folder2 = f"./data/azim/phy_tangential/"
...
# データの読み込み
data_all_u = np.memmap(f"{config.input_folder}ms_phy_du.grd", dtype=">f4", mode="r",
                    shape=(config.nt, config.nz, config.ny, config.nx))
data_all_v = np.memmap(f"{config.input_folder}ms_phy_dv.grd", dtype=">f4", mode="r",
                    shape=(config.nt, config.nz, config.ny, config.nx))
