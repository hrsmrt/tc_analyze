...
# データの読み込み
data_all_phy_u = np.memmap(f"{config.input_folder}ms_phy_du.grd", dtype=">f4", mode="r",
                    shape=(config.nt, config.nz, config.ny, config.nx))
data_all_phy_v = np.memmap(f"{config.input_folder}ms_phy_dv.grd", dtype=">f4", mode="r",
                    shape=(config.nt, config.nz, config.ny, config.nx))
data_all_tb_u = np.memmap(f"{config.input_folder}ms_tb_du.grd", dtype=">f4", mode="r",
                    shape=(config.nt, config.nz, config.ny, config.nx))
data_all_tb_v = np.memmap(f"{config.input_folder}ms_tb_dv.grd", dtype=">f4", mode="r",
                    shape=(config.nt, config.nz, config.ny, config.nx))
