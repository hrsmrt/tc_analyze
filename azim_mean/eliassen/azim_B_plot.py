...
sample_data = np.load(f"./data/azim/eliassen/B/t{str(config.t_first).zfill(3)}.npy")
nr = sample_data.shape[1]
R_MAX = nr * config.dx
r_mesh, z_mesh = grid.create_radial_vertical_meshgrid(R_MAX)

output_folder = "./fig/azim/eliassen/B/"
os.makedirs(output_folder, exist_ok=True)


def process_t(t):
    data = np.load(f"./data/azim/eliassen/B/t{str(t).zfill(3)}.npy")
