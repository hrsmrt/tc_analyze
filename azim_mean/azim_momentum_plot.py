...
input_folder = "./data/azim/momentum/"
...
def process_t(t):
    ...
    c = ax.contourf(X*1e-3, Y*1e-3, data, levels=np.arange(0,2.8e7,0.4e7), cmap="rainbow", extend="both")
    cbar = fig.colorbar(c,ax=ax)
    cbar.set_ticks([0,2.8e7])
    ax.set_ylim([0,20])
    ax.set_title(f"角運動量 t = {time_list[t]} hour")
    ax.set_xlabel("半径 [km]")
    ax.set_ylabel("高度 [km]")
