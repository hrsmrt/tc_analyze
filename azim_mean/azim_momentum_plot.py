...
sample_data = np.load(f"{config.get_data_path('azim', 'momentum')}/t{str(config.t_first).zfill(3)}.npy")
...
def process_t(t):
    momentum_data = np.load(f"{config.get_data_path('azim', 'momentum')}/t{str(t).zfill(3)}.npy")
