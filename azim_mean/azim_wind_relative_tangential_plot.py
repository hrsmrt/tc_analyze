...
folder = "./fig/azim/wind_relative_tangential/"
...
def process_t(t):
  ...
  if t < 96:
    ...
    c = ax.contourf(X,Y,data,cmap="bwr",levels=np.arange(-60,70,10),extend="both")
    cbar = fig.colorbar(c, ax=ax)
    cbar.set_ticks([-60,0,60])
  ax.set_title(f"方位角平均接線風 t = {time_list[t]} hour")
  ax.set_ylim([0, 20])
