...
def calculate_center_velocity(
    center_x_list: np.ndarray,
    center_y_list: np.ndarray,
    dt: float
) -> Tuple[np.ndarray, np.ndarray]:
    """
    台風中心の移動速度を2次中心差分で計算
    ...
    Returns
    -------
    center_u_list : np.ndarray
        台風中心のx方向移動速度 [m/s]
    center_v_list : np.ndarray
        台風中心のy方向移動速度 [m/s]

    Notes
    -----
    - 内点: 2次中心差分 u[i] = (x[i+1] - x[i-1]) / (2*dt)
    - 端点: 前進/後退差分を使用
    - 相対風の計算に使用する中心移動速度を導出

    Examples
    --------
    >>> center_x = np.array([0, 100, 250, 450])  # [m]
    >>> center_y = np.array([0, 80, 180, 300])   # [m]
    >>> dt = 3600  # 1時間 [s]
    >>> u, v = calculate_center_velocity(center_x, center_y, dt)
    >>> print(u)  # x方向移動速度 [m/s]
    """
    nt = len(center_x_list)
    center_u_list = np.zeros(nt)
    center_v_list = np.zeros(nt)

    # 内点: 2次中心差分
    center_u_list[1:-1] = (center_x_list[2:] - center_x_list[:-2]) / (2 * dt)
    center_v_list[1:-1] = (center_y_list[2:] - center_y_list[:-2]) / (2 * dt)

    # 端点: 前進差分
    center_u_list[0] = (center_x_list[1] - center_x_list[0]) / dt
    center_v_list[0] = (center_y_list[1] - center_y_list[0]) / dt

    # 端点: 後退差分
    center_u_list[-1] = (center_x_list[-1] - center_x_list[-2]) / dt
    center_v_list[-1] = (center_y_list[-1] - center_y_list[-2]) / dt

    return center_u_list, center_v_list
