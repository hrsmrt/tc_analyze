...
radial_max = 500e3
extent_x = int(radial_max / dx)
extent_y = int(radial_max / dy)

center_x_list = np.loadtxt("./data/ss_slp_center_x.txt")
center_y_list = np.loadtxt("./data/ss_slp_center_y.txt")

os.makedirs(str(f"./fig/3d/vortex_region/relative_wind_radial/"),exist_ok=True)
...
def process_t(t):
    ...
    for z in z_list:
        ...
        cf = ax.contourf(X_cut,Y_cut,data_masked,cmap="bwr",levels=np.arange(-30,35,5),extend='both')
        ...
        fig.savefig(f"./fig/3d/vortex_region/relative_wind_radial/z{str(z).zfill(2)}/t{str(time_list[t]).zfill(3)}.png")
        plt.close()
