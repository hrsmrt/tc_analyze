...
from utils.plotting import parse_style_argument, create_custom_colormap
...
def process_t(t):
  for z in z_list:
    ...
    match varname:
        case "ms_u":
          c = ax.contourf(grid.X, grid.Y,data,levels=np.arange(-40,45,5),cmap="bwr",extend="both")
        case "ms_v":
          c = ax.contourf(grid.X, grid.Y,data,levels=np.arange(-40,45,5),cmap="bwr",extend="both")
        case "ms_w":
          c = ax.contourf(grid.X, grid.Y,data,levels=np.arange(-4,4.5,0.5),cmap="bwr",extend="both")
        case "ms_tem":
          if z == 0:
             c = ax.contourf(grid.X, grid.Y,data,levels=np.arange(295,305,1),cmap="rainbow",extend="both")
          else:
             c = ax.contourf(grid.X, grid.Y,data,cmap="rainbow",extend="both")
        case "ms_rh":
            c = ax.contourf(grid.X, grid.Y,data,levels=np.arange(0,1.2,0.1),cmap="rainbow",extend="both")
        case "ms_qv":
            if z == 0:
                c = ax.contourf(grid.X, grid.Y,data,levels=np.arange(0.005,0.027,0.002),cmap="rainbow",extend="both")
            else:
                c = ax.contourf(grid.X, grid.Y,data,cmap="rainbow",extend="both")
        case _:
