...
def process_t(t):
    ...
    for z in range(config.nz):
        ...
        du_dy[0, :config.nx//2] = (data_u[z, 1, :config.nx//2] - data_u[z, -1, config.nx//2:]) / (2 * config.dy)
        du_dy[0, config.nx//2:] = (data_u[z, 1, config.nx//2:] - data_u[z, -1, :config.nx//2]) / (2 * config.dy)
        du_dy[-1, :config.nx//2] = (data_u[z, 0, :config.nx//2] - data_u[z, -2, config.nx//2:]) / (2 * config.dy)
        du_dy[-1, config.nx//2:] = (data_u[z, 0, config.nx//2:] - data_u[z, -2, :config.nx//2]) / (2 * config.dy)
        vor[z] = dv_dx - du_dy
