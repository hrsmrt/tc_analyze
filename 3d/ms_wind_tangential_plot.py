...
os.makedirs(str(f"./fig/3d/vortex_region/wind_tangential/"),exist_ok=True)
...
for z in z_list:
  os.makedirs(f"./fig/3d/vortex_region/wind_tangential/z{str(z).zfill(2)}",exist_ok=True)

vgrid = np.loadtxt(f"{script_dir}/../../database/vgrid/vgrid_c74.txt")

def process_t(t):
    data_t = np.load(f"./data/3d/wind_tangential/t{str(t).zfill(3)}.npy")
    ...
    for z in z_list:
        ...
        c = ax.contourf(X_cut,Y_cut,data_cut,cmap="bwr",levels=np.arange(-30,35,5),extend='both')
        fig.colorbar(c, ax=ax)
        ax.set_xticks(
                [0, extent_x * dx, 2 * extent_x * dx],
                [int(-extent_x * dx * 1e-3), "0", int(extent_x * dx * 1e-3)],
            )
        ax.set_yticks(
            [0, extent_y * dy, 2 * extent_y * dy],
            [int(-extent_y * dy * 1e-3), "0", int(extent_y * dy * 1e-3)],
        )
        ax.set_title(f"t={t}h, z={round(vgrid[z]*1e-3, 1):.1f}km")
        ax.set_xlabel("x [km]")
        ax.set_ylabel("y [km]")
        ax.set_aspect("equal", "box")
        fig.savefig(f"./fig/3d/vortex_region/wind_tangential/z{str(z).zfill(2)}/t{str(time_list[t]).zfill(3)}.png")
        plt.close()
