...
os.makedirs(str("./fig/3d/vortex_region/wind_tangential/"), exist_ok=True)
...
for z in z_list:
    os.makedirs(
        f"./fig/3d/vortex_region/wind_tangential/z{str(z).zfill(2)}", exist_ok=True
    )
...
def process_t(t):
    ...
    for z in z_list:
        ...
        fig.savefig(
            f"./fig/3d/vortex_region/wind_tangential/z{str(z).zfill(2)}/t{str(config.time_list[t]).zfill(3)}.png"
        )
        plt.close()
