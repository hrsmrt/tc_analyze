...
os.makedirs(str(f"./fig/3d/vortex_region/divergence/"),exist_ok=True)

X_cut, Y_cut = grid.get_vortex_region_meshgrid(extent)

z_list = [0,9,17,23,29,36,42,48,54,60]
for z in z_list:
  os.makedirs(f"./fig/3d/vortex_region/divergence/z{str(z).zfill(2)}",exist_ok=True)

vgrid = np.loadtxt(f"{config.vgrid_filepath}")

def process_t(t):
    data_t = np.load(f"./data/3d/divergence/div_t{str(t).zfill(3)}.npy")
    ...
    for z in z_list:
        ...
        fig.savefig(f"./fig/3d/vortex_region/divergence/z{str(z).zfill(2)}/t{str(config.time_list[t]).zfill(3)}.png")
        plt.close()
